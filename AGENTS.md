...
## Domain Rules
- Every API method cites its Salesforce documentation URL in the docstring.
- Destructive paths (record delete, collections delete, bulk delete/hardDelete,
  DELETE composite subrequests) MUST stay behind `allow_destructive`
  (`SALESFORCE_ALLOW_DESTRUCTIVE`, default False) — enforce in the API layer,
  not just the tools.
- Never log or raise raw tokens/secrets — route messages through
  `SalesforceAuth.redact`.
- Keep result surfaces capped: SOQL per-call record cap, bulk results byte cap.
- Name code from its purpose, never `wave0`/`phase2`/`v2`.
...
edit the canonical checkout** (`/home/apps/workspace/agent-packages/agents/<repo>`) — a
...
# raw-git fallback:
git -C agent-packages/agents/<repo> checkout main
git -C agent-packages/agents/<repo> worktree add /home/apps/worktrees/<repo>/<branch> -b <branch>
```
